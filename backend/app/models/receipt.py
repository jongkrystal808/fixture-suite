...
class ReceiptCreate(ReceiptBase):
    ...
    def validate_datecode(cls, v, values):
        if values.get("type") == RecordType.DATECODE:
            if not v:
                raise ValueError("datecode 收料需要 datecode")
        return v
    ...
    def validate_quantity(cls, v, values):
        if values.get("type") == RecordType.DATECODE:
            if not v or v <= 0:
                raise ValueError("datecode 收料需要 quantity 且必須大於 0")
        return v
...
class ReturnCreate(ReturnBase):
    ...
    serial_end: Optional[str] = None
    serials: Optional[str] = None
    datecode: Optional[str] = None  # ★ 新增
    quantity: Optional[int] = None   # ★ 新增
